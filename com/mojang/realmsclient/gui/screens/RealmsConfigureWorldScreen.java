...
public class RealmsConfigureWorldScreen extends RealmsScreenWithCallback<WorldTemplate> {
   ...
   public void init() {
      ...
      this.buttonsAdd(
         this.optionsButton = new RealmsHideableButton(
            5, this.leftButton(0), RealmsConstants.row(13) - 5, this.default_button_width + 10, 20, getLocalizedString("mco.configure.world.buttons.options")
         )
      );
      this.buttonsAdd(
         this.backupButton = new RealmsHideableButton(
            6, this.leftButton(1), RealmsConstants.row(13) - 5, this.default_button_width + 10, 20, getLocalizedString("mco.configure.world.backup")
         )
      );
      this.buttonsAdd(
         this.resetWorldButton = new RealmsHideableButton(
            ...
            this.default_button_width + 10,
            20,
            getLocalizedString("mco.configure.world.buttons.resetworld")
         )
      );
      this.buttonsAdd(
         ...
         for(Entry<Integer, RealmsOptions> entry : this.serverData.slots.entrySet()) {
            if (((RealmsOptions)entry.getValue()).templateImage != null && ((RealmsOptions)entry.getValue()).templateId != -1L) {
               this.drawSlotFrame(
                  ...
                  entry.getKey(),
                  ((RealmsOptions)entry.getValue()).templateId,
                  ((RealmsOptions)entry.getValue()).templateImage,
                  ((RealmsOptions)entry.getValue()).empty
               );
            } else {
               this.drawSlotFrame(
                  this.frame(entry.getKey()),
      ...
      if (buttonNum == 0) {
         ...
         if (this.hoveredSlot != -1) {
            if (this.hoveredSlot < 4) {
               ...
               if (((RealmsOptions)this.serverData.slots.get(this.hoveredSlot)).empty) {
   ...
   public void saveSlotSettings(RealmsOptions options) {
      RealmsOptions oldOptions = (RealmsOptions)this.serverData.slots.get(this.serverData.activeSlot);
