...
public class EditOnlineWorldScreen extends RealmsScreen implements RealmsConfirmResultListener {
   ...
   public EditOnlineWorldScreen(RealmsScreen configureWorldScreen, RealmsScreen onlineScreen, McoServer serverData) {
   ...
   public void init() {
      this.column1_x = this.width() / 4;
      this.column_width = this.width() / 4 - 2;
      this.column2_x = this.width() / 2 + 4;
      Keyboard.enableRepeatEvents(true);
      this.buttonsClear();
      this.buttonsAdd(
         this.moreSettingsButton = newButton(3, this.column1_x, 122, this.width() / 2, 20, getLocalizedString("mco.configure.world.buttons.moreoptions"))
      );
      this.buttonsAdd(
         this.doneButton = newButton(
            0, this.column1_x, this.height() / 4 + 120 + 22, this.column_width, 20, getLocalizedString("mco.configure.world.buttons.done")
         )
      );
      this.buttonsAdd(newButton(1, this.column2_x, this.height() / 4 + 120 + 22, this.column_width, 20, getLocalizedString("gui.cancel")));
      this.nameEdit = this.newEditBox(this.width() / 2 - 106, 56, 212, 20);
      this.nameEdit.setFocus(true);
      this.nameEdit.setMaxLength(32);
      this.nameEdit.setValue(this.serverData.getName());
      this.descEdit = this.newEditBox(this.width() / 2 - 106, 96, 212, 20);
   ...
   public void buttonClicked(RealmsButton button) {
      if (button.active()) {
         if (button.id() == 1) {
            ...
            this.saveServerData();
            Realms.setScreen(new OnlineWorldSettingsSubScreen(this, this.serverData));
         }
