...
public class y extends RealmsScreen {
   ...
   public y(RealmsScreen lastScreen, Backup backup) {
      this.c = lastScreen;
      this.e = backup;
   ...
   class a extends RealmsSimpleScrolledSelectionList {
      public a() {
         super(y.this.width(), y.this.height(), 32, y.this.height() - 64, 36);
      }

      public int getItemCount() {
         return y.this.e.changeList.size();
      }
      ...
      protected void renderItem(int i, int x, int y, int h, Tezzelator t, int mouseX, int mouseY) {
         String key = (String)y.this.f.get(i);
         y.this.drawString(key, this.width() / 2 - 40, y, 10526880);
         String metadataValue = (String)y.this.e.changeList.get(key);
         y.this.drawString(y.this.a(key, metadataValue), this.width() / 2 - 40, y + 12, 16777215);
      }
   }
}
